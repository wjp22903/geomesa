...
object GeoserverClientService extends Logging {
  ...
  def registrationData(surfaceId: String, resolution: Int, styleName: String, connectConfig: Map[String, Option[String]]):
    RegistrationData = {
    val url = coverageURL(surfaceId, resolution, connectConfig)
    val coverageName = surfaceId
    val storeName = connectConfig(IngestRasterParams.CATALOG_TABLE).get + ":" + coverageName
    RegistrationData(url, storeName, coverageName, styleName)
  }

  def coverageURL(surfaceId: String, resolution: Int, params: Map[String, Option[String]]): String = {
    val zookeepers = params(IngestRasterParams.ZOOKEEPERS).get
    val instance = params(IngestRasterParams.ACCUMULO_INSTANCE).get
    val tableName = params(IngestRasterParams.CATALOG_TABLE).get
    ...
    s"accumulo://$user:$password@$instance/" +
      s"$tableName/$surfaceId/#resolution=$resolution" +
      s"#zookeepers=$zookeepers#auths=$auths"
  }
  ...
  def registerSurface(surfaceId: String,
    ...
    Await.ready(registrationActor ? registrationData(surfaceId, precision,
