...
object SpatioTemporalIntersectingIteratorTest {
...
import SpatioTemporalIntersectingIteratorTest._
...
class SpatioTemporalIntersectingIteratorTest extends Specification {
  ...
  def runMockAccumuloTest(label: String,
    ...
    val c: Connector = TestData.setupMockAccumuloTable(entries)
    val bs: () => BatchScanner = { () => c.createBatchScanner(TEST_TABLE, TEST_AUTHORIZATIONS, 5) }
