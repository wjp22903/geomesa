...
function ($timeout) {
    ...
    // b     - record index of first item in the sequence (typically 0)
    // e     - record index of just beyond last item in the sequence (typically
    //         the number of records)
    // koI32 - sort key offset within record in units of Int32 (4-bytes)
    // szF64 - size of a record in units of Float64 (8-bytes)
    //
    this.sortedBinRec = function (vi32, b, e, koI32, szF64) {
        b |= 0;
        e |= 0;
        koI32 |= 0;
        szF64 |= 0;
        var szI32=(szF64<<1)|0;
        var ib=0, ie=0, v=0, lastv=0;
    ...
    //
    this.sortBinRec = function (vi32, vf64, si32, spI32, koI32, szF64, completionFunc, completionArg) {
        spI32 |= 0;
        koI32 |= 0;
        szF64 |= 0;
