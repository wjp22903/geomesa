/*
 * Custom checkbox control to allow users to switch between different layers on the map.
 * Copied from leaflet-src.js and modified.
 */
AirTrackerLayerControl = L.Control.extend({
	options: {
		collapsed: true,
		position: 'topright',
		autoZIndex: true
	...
	initialize: function (baseLayers, overlays, colors, options) {
		...
		for (i in overlays) {
			this._addLayer(overlays[i], i, true, colors[i]);
		}
	},
	...
	addOverlay: function (layer, name, color) {
		this._addLayer(layer, name, true, color);
		this._update();
		return this;
	},
	...
	_addLayer: function (layer, name, overlay, color) {
		var id = L.stamp(layer);
...
airTrackerLayerControl = function (baseLayers, overlays, colors, options) {
